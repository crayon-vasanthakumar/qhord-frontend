...
exports.useFreeDemoStack = useFreeDemoStack;
exports.mapManifestTool = mapManifestTool;
exports.getEnvToolsForDemoStack = getEnvToolsForDemoStack;
/** When true, manifest steps for Apollo/Clay/Smartlead run via Hunter/Brevo instead. */
function useFreeDemoStack() {
...
function mapManifestTool(tool, action) {
    if (!useFreeDemoStack())
        return tool;
    if (tool === 'apollo' && SOURCE_ACTIONS.has(action))
        return 'hunter';
    if (tool === 'clay' && ENRICH_ACTIONS.has(action))
        return 'bettercontacts';
    if (tool === 'smartlead' && SEND_ACTIONS.has(action))
        return 'brevo';
    if (tool === 'heyreach' && SEND_ACTIONS.has(action))
        return 'brevo';
    if (tool === 'instantly' && SEND_ACTIONS.has(action))
        return 'brevo';
    return tool;
}
function getEnvToolsForDemoStack() {
    const tools = [];
    if (process.env.HUNTER_API_KEY?.trim())
        tools.push('Hunter');
    if (process.env.BREVO_API_KEY?.trim())
        tools.push('Brevo');
    if (process.env.BETTERCONTACTS_API_KEY?.trim())
        tools.push('BetterContacts');
    if (process.env.CALENDLY_API_KEY?.trim())
        tools.push('Calendly');
    if (process.env.SMARTLEAD_API_KEY?.trim())
        tools.push('Smartlead');
    if (process.env.HEYREACH_API_KEY?.trim())
        tools.push('HeyReach');
    if (process.env.INSTANTLY_API_KEY?.trim())
        tools.push('Instantly');
    if (process.env.HUBSPOT_API_KEY?.trim())
        tools.push('HubSpot');
    if (process.env.SALESFORCE_ACCESS_TOKEN?.trim())
