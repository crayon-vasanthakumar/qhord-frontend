...
router.get('/metrics', async (req, res) => {
    try {
        ...
        const campaignStats = await prisma_1.prisma.$queryRaw `
      SELECT 
...
router.get('/priorities', async (req, res) => {
    try {
        ...
        const campaigns = await prisma_1.prisma.$queryRaw `
      SELECT 
        id,
        name,
        status,
      ...
      LIMIT 5
    `;
        const priorities = campaigns.map((campaign, index) => {
...
router.get('/recommendations', async (req, res) => {
    try {
        ...
        const activeCampaigns = await prisma_1.prisma.$queryRaw `
      SELECT 
        id,
        name,
        status,
      ...
      LIMIT 3
    `;
        const recommendations = activeCampaigns.map((campaign) => {
            const entity = campaign.name || `Campaign ${campaign.id.substring(0, 8)}`;
...
router.get('/live-activity', async (req, res) => {
    try {
        ...
        const recentCampaigns = await prisma_1.prisma.$queryRaw `
      SELECT 
        id,
        name,
        status,
      ...
      LIMIT 5
    `;
        const liveActivity = recentCampaigns.map((campaign) => {
            const entity = campaign.name || `Campaign ${campaign.id.substring(0, 8)}`;
...
router.get('/health-table', async (req, res) => {
    try {
      ...
      LIMIT 10
    `;
        const healthTable = campaigns.map((campaign) => {
