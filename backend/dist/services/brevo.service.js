...
class BrevoService {
    ...
    async createList(payload) {
        const res = await this.client.post('/contacts/lists', { name: payload.name || `Qhord ${Date.now()}`, folderId: 1 }, { headers: this.headers() });
        return res.data;
    }
    async createContact(payload) {
        const res = await this.client.post('/contacts', {
            email: payload.email,
            attributes: {
...
<br/><p>Best,<br/>{{ contact.OWNER_NAME || 'Qhord Team' }}</p></body></html>`;
        ...
        const res = await this.client.post('/emailCampaigns', {
            name,
            subject,
    ...
    async sendTransactional(payload) {
        ...
        for (const recipient of (payload.to || []).slice(0, 10)) {
            const res = await this.client.post('/smtp/email', {
                sender: { name: senderName, email: senderEmail },
