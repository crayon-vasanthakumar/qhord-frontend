...
class WorkflowEngine {
    ...
    async triggerWorkflow(triggerEvent, clientId, payload, operatorId) {
        try {
            ...
            const activeWorkflows = await prisma_1.prisma.workflow.findMany({
                where: {
                    client_id: clientId,
            ...
            for (const wf of activeWorkflows) {
                const syncPipeline = process.env.QHORD_SYNC_PIPELINE === 'true' || (0, demo_stack_1.useFreeDemoStack)();
                if (syncPipeline) {
                    ...
                    await workflow_queue_1.workflowQueue.add(`wf-${wf.id}-${Date.now()}`, {
                        workflowId: wf.id,
                        payload,
    ...
    async executeWorkflow(workflowId, payload, operatorId) {
        ...
        const run = await prisma_1.prisma.workflowRun.create({
            data: {
        ...
        try {
            ...
            const filterResult = await this.evaluateFilters(workflow.enrollment_filters, targetRecord, workflow.client_id);
            await prisma_1.prisma.workflowRun.update({
                where: { id: run.id },
                data: { filter_result: filterResult },
            });
            ...
            const guardrailResult = await this.evaluateGuardrails(workflow.guardrails, targetRecord, workflow.client_id);
            await prisma_1.prisma.workflowRun.update({
                where: { id: run.id },
                data: { guardrail_result: guardrailResult },
            });
            ...
            const executionResult = await this.executePath(workflow.path_actions, targetRecord, workflow.client_id, operatorId);
            await prisma_1.prisma.workflowRun.update({
                where: { id: run.id },
                data: {
                    status: 'completed',
    ...
    async loadTargetRecord(clientId, targetType, payload) {
        ...
        if (targetType === 'people') {
            let lead = null;
            if (payload.email) {
                lead = await prisma_1.prisma.lead.findFirst({ where: { client_id: clientId, email: payload.email } });
            }
            else if (payload.record_id) {
                lead = await prisma_1.prisma.lead.findFirst({ where: { id: payload.record_id } });
            }
        ...
        if (targetType === 'deals') {
            ...
            if (payload.record_id) {
                deal = await prisma_1.prisma.deal.findFirst({ where: { id: payload.record_id } });
            }
            else if (payload.name) {
                deal = await prisma_1.prisma.deal.findFirst({ where: { client_id: clientId, name: payload.name } });
            }
    ...
    async evaluateFilters(filters, record, clientId) {
    ...
    // Evaluate Guardrails
    async evaluateGuardrails(guardrails, record, clientId) {
    ...
    // Execute Path preset
    async executePath(path, record, clientId, operatorId) {
        const presetName = typeof path === 'string' ? path : path?.name || '';
        ...
        if (presetName === 'Email-first cadence') {
            ...
            const leadAccount = await (0, ensure_tool_accounts_1.findToolAccount)(clientId, 'smartlead') ||
                await (0, ensure_tool_accounts_1.findToolAccount)(clientId, 'apollo') ||
                await (0, ensure_tool_accounts_1.findToolAccount)(clientId, 'instantly');
            if (!leadAccount) {
            ...
            const reachAccount = await (0, ensure_tool_accounts_1.findToolAccount)(clientId, 'heyreach');
            if (!reachAccount) {
                throw new Error('No HeyReach account connected for LinkedIn-first actions.');
            }
            toolUsed = 'heyreach';
            actionExecuted = 'heyreach: send_message';
            const service = new heyreach_service_1.HeyReachService(reachAccount.api_key_encrypted);
            const campaigns = await service.fetchCampaigns();
            const campaignId = campaigns[0]?.id || '1';
            await service.enrollLead({
                campaign_id: campaignId,
            ...
            if (record.email) {
                ...
                const enrichmentAccount = await (0, ensure_tool_accounts_1.findToolAccount)(clientId, 'clay') ||
                    await (0, ensure_tool_accounts_1.findToolAccount)(clientId, 'bettercontacts');
    ...
    async getDefaultOperatorId(clientId) {
        const client = await prisma_1.prisma.client.findUnique({
            where: { id: clientId },
            select: { created_by_operator_id: true },
        });
        if (client?.created_by_operator_id)
            return client.created_by_operator_id;
        const fallbackOp = await prisma_1.prisma.operator.findFirst();
