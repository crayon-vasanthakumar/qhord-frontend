...
async function testWorkflow() {
    ...
    let operator = await prisma_1.prisma.operator.findFirst();
    if (!operator) {
        operator = await prisma_1.prisma.operator.create({
            data: {
    ...
    const workflow = await prisma_1.prisma.campaignWorkflow.create({
        data: {
    ...
    const apolloNode = await prisma_1.prisma.workflowNode.create({
        data: {
    ...
    const betterContactNode = await prisma_1.prisma.workflowNode.create({
        data: {
            workflow_id: workflow.id,
    ...
    const smartleadNode = await prisma_1.prisma.workflowNode.create({
        data: {
            workflow_id: workflow.id,
            node_type: 'action',
            tool: 'smartlead',
    ...
    await campaign_workflow_engine_1.campaignWorkflowEngine.startWorkflowRun(workflow.id, lead.id);
    // 6. Verify run step results in the database
    const run = await prisma_1.prisma.campaignWorkflowRun.findFirst({
    ...
    await prisma_1.prisma.campaignWorkflowRun.deleteMany({ where: { campaign_workflow_id: workflow.id } });
    await prisma_1.prisma.workflowEdge.deleteMany({ where: { workflow_id: workflow.id } });
    await prisma_1.prisma.workflowNode.deleteMany({ where: { workflow_id: workflow.id } });
    await prisma_1.prisma.campaignWorkflow.delete({ where: { id: workflow.id } });
    await prisma_1.prisma.lead.delete({ where: { id: lead.id } });
