...
export class ValidatorNode {
  async invoke(state: ValidatorState): Promise<Partial<ValidatorState>> {
    try {
      if (!state.manifest) {
  ...
  private validateManifestStructure(manifest: CampaignManifest, errors: string[]): void {
    if (!manifest.steps) {
  ...
  private validateToolAvailability(manifest: CampaignManifest, activeTools: string[], warnings: string[]): void {
  ...
  private validateGuardrails(manifest: CampaignManifest, intent: CampaignIntent | undefined, errors: string[], warnings: string[]): void {
    // Check for zero-delay emails
    const emailSteps = manifest.steps.filter(s =>
      ['Smartlead', 'Instantly', 'Lemlist'].includes(s.tool) &&
      ['send_campaign', 'send_emails'].includes(s.action)
    );

    for (const step of emailSteps) {
      // Check if there's a warmup delay before this step
      const warmupSteps = manifest.steps.filter(s =>
        s.tool === 'System' &&
        s.action === 'wait' &&
        s.order < step.order
      );
  ...
  private validateRateLimits(manifest: CampaignManifest, errors: string[], warnings: string[]): void {
  ...
  private validateDuplicatePrevention(manifest: CampaignManifest, warnings: string[]): void {
    // Check if campaign might create duplicates
    const sourcingSteps = manifest.steps.filter(s =>
      ['Apollo', 'ZoomInfo', 'Cognism'].includes(s.tool) &&
    ...
    const crmSteps = manifest.steps.filter(s =>
      ['HubSpot', 'Salesforce', 'Pipedrive'].includes(s.tool)
    );
  ...
  private applyAutoFixes(manifest: CampaignManifest, warnings: string[]): CampaignManifest {
    const fixedManifest: CampaignManifest = {
      ...manifest,
    ...
    const emailStepsWithoutWarmup = fixedManifest.steps.filter(step => {
      if (!['Smartlead', 'Instantly', 'Lemlist'].includes(step.tool)) return false;

      const hasWarmup = fixedManifest.steps.some(s =>
        s.tool === 'System' &&
        s.action === 'wait' &&
        s.order < step.order
      );
      ...
      fixedManifest.steps.forEach(step => {
        if (step.order > warmupStep.order) {
          step.dependencies = step.dependencies.map(dep =>
            dep === firstEmailStep.id ? warmupStep.id : dep
          );
        }
      });
