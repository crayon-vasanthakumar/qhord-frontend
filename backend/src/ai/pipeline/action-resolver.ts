import type { ManifestStepLike, PipelineContext, ResolvedExecutionStep } from './pipeline-types';
import { normalizeToolName } from './pipeline-types';
import { mapManifestTool } from '../../config/demo-stack';
import { leadsToClayRows, leadsToSmartleadList, leadsToBrevoContacts, leadsToBetterContacts } from './pipeline-leads';
 ...
 */
export function resolveManifestSteps(steps: ManifestStepLike[]): ResolvedExecutionStep[] {
...
function resolveOneStep(step: ManifestStepLike): ResolvedExecutionStep[] {
  const rawTool = normalizeToolName(step.tool);
  ...
  const tool = mapManifestTool(rawTool, action);
  ...
  if (tool === 'brevo') {
    if (SEND_ACTIONS.has(action)) {
      const campaignName =
        String(params.campaign_name || params.name || params.subject || 'Qhord Outreach');
  ...
  if (tool === 'heyreach') {
    const mapped =
      action === 'send_connection_requests' ? 'add_leads_to_campaign' : action;
    return [
      {
        manifestStepId: id,
        tool: 'heyreach',
...
export function applyPipelineToPayload(
  ...
  if (resolved.tool === 'smartlead' && resolved.action === 'add_leads_to_campaign') {
    const list = leadsToSmartleadList(ctx.leads);
  ...
  if (resolved.tool === 'brevo' && resolved.action === 'sync_contacts' && payload._from_pipeline) {
    ...
    payload.contacts = leadsToBrevoContacts(ctx.leads);
    delete payload._from_pipeline;
  }
  ...
  if (resolved.tool === 'bettercontacts' && resolved.action === 'enrich_contacts' && payload._from_pipeline) {
    payload.contacts = leadsToBetterContacts(ctx.leads);
    delete payload._from_pipeline;
  }
...
function buildQueryFromFilters(filters: Record<string, unknown>): string {
  ...
  if (Array.isArray(filters.titles)) parts.push((filters.titles as string[]).join(' OR '));
