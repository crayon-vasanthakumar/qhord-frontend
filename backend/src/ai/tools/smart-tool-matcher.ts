import { ToolRegistry, ToolCapability, ToolSequence, TOOL_REGISTRY, TOOL_SEQUENCES } from './tool-registry';
...
export interface IntentAnalysis {
  ...
  channels: string[];
  tools_needed: string[];
  recommended_sequence: ToolSequence;
  alternative_sequences: ToolSequence[];
  total_cost: number;
}

export class SmartToolMatcher {
  static analyzeUserIntent(userInput: string): IntentAnalysis {
  ...
  static extractPrimaryGoal(input: string): string {
    const goal_patterns = {
  ...
  static extractTargetAudience(input: string): string {
  ...
  static extractVolume(input: string): number {
  ...
  static extractChannels(input: string): string[] {
    const channels = [];
    
    if (/\b(email|mail|outreach)\b/.test(input)) channels.push('email');
    if (/\b(sms|text|message)\b/.test(input)) channels.push('sms');
    if (/\b(linkedin|social)\b/.test(input)) channels.push('social');
    if (/\b(call|phone)\b/.test(input)) channels.push('call');
    
    return channels.length > 0 ? channels : ['email']; // Default
  }

  static determineToolsNeeded(goal: string, channels: string[]): string[] {
  ...
  static getRecommendedSequence(tools: string[], goal: string): ToolSequence {
    // Find best matching sequence
    const availableSequences = TOOL_SEQUENCES.filter(seq => 
      seq.tools.every(tool => tools.includes(tool))
    );
    ...
    availableSequences.forEach(sequence => {
      ...
      if (goalLower.includes('email') && useCase.includes('email')) score += 3;
      if (goalLower.includes('sms') && useCase.includes('sms')) score += 3;
      if (goalLower.includes('enrich') && useCase.includes('enrich')) score += 3;
      if (goalLower.includes('multi') && useCase.includes('multi')) score += 3;
      if (goalLower.includes('lead') && useCase.includes('lead')) score += 2;
  ...
  static getAlternativeSequences(tools: string[], goal: string): ToolSequence[] {
    const alternatives = TOOL_SEQUENCES.filter(seq => 
      seq.tools.every(tool => tools.includes(tool)) && seq.name !== this.getRecommendedSequence(tools, goal).name
    );

    return alternatives.slice(0, 2); // Return top 2 alternatives
  }

  static createDefaultSequence(tools: string[], goal: string): ToolSequence {
    const actions: string[] = [];
    
    tools.forEach(tool => {
      const toolCap = TOOL_REGISTRY[tool];
      if (toolCap) {
    ...
    return {
      ...
      tools,
      actions,
      total_credit_cost: ToolRegistry.calculateSequenceCost(tools),
      use_case: goal
    };
  }

  static calculateTotalCost(sequence: ToolSequence): number {
    return sequence.total_credit_cost;
  }

  static matchToolsToIntent(intent: string, availableTools: string[]): ToolMatch[] {
  ...
  static validateToolSequence(sequence: string[], availableTools: string[]): {
    valid: boolean;
    errors: string[];
    warnings: string[];
  } {
  ...
  static suggestOptimalSequence(intent: string, availableTools: string[]): {
    recommended: ToolSequence;
    alternatives: ToolSequence[];
    reasoning: string;
  } {
