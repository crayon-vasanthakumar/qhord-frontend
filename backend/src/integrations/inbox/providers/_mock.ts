import { ProviderConversation, MessageSendResult } from '../types';
 ...
 */
export function buildMockConversations(toolId: string): ProviderConversation[] {
  const now = Date.now();
  return SEED.map((s, i) => {
 ...
 */
export function mockSend(toolId: string): MessageSendResult {
  return {
    success: true,
