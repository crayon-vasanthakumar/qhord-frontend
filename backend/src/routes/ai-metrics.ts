...
// AI SDR Metrics
router.get('/ai-sdr/metrics', async (req: Request, res: Response) => {
  try {
    ...
    for (const row of executionRows) {
      ...
      const payload: any = row.response_payload || {};
      if (typeof payload.replies === 'number') totalReplies += payload.replies;
      if (typeof payload.meetings === 'number') totalMeetings += payload.meetings;
      if (typeof payload.unsubscribes === 'number') totalUnsubscribes += payload.unsubscribes;
      if (row.status === 'error') bounced += 1;
    }
...
// AI Operator Metrics
router.get('/ai-operator/metrics', async (req: Request, res: Response) => {
  try {
    const operatorId = req.user!.id;
...
// AI Engine Metrics
router.get('/ai-engine/metrics', async (req: Request, res: Response) => {
  try {
    const operatorId = req.user!.id;
...
router.get('/dashboard/metrics', async (req: Request, res: Response) => {
  try {
    ...
    for (const row of executions) {
      if (row.action === 'send_campaign' || row.action === 'add_leads_to_campaign') totalEmails += 1;
      const payload: any = row.response_payload || {};
      if (typeof payload.replies === 'number') totalReplies += payload.replies;
      if (typeof payload.meetings === 'number') totalMeetings += payload.meetings;
    }
