...
import { getInboxProvider, canToolSend, INBOX_PROVIDER_IDS } from '../integrations/inbox/registry';
import { InboxProviderContext } from '../integrations/inbox/types';
import { getConnectedInboxProviders, syncClientInbox } from '../services/inbox-sync.service';
...
router.get('/:clientId/inbox/conversations', async (req: Request, res: Response) => {
...
router.get(
  '/:clientId/inbox/conversations/:conversationId/messages',
...
router.post(
  '/:clientId/inbox/conversations/:conversationId/send',
  async (req: Request, res: Response) => {
    const { clientId, conversationId } = req.params;
    ...
    try {
      ...
      const provider = getInboxProvider(conversation.tool_id);
      ...
      try {
        ...
        if (!result.success) {
          ...
          console.error(
            `[unified-inbox] send failed via ${conversation.tool_id}:`,
            result.error,
          );
...
router.post('/:clientId/inbox/sync', async (req: Request, res: Response) => {
  const { clientId } = req.params;
  try {
    const result = await syncClientInbox(clientId);
...
router.patch(
  '/:clientId/inbox/conversations/:conversationId/status',
  async (req: Request, res: Response) => {
    const { clientId, conversationId } = req.params;
