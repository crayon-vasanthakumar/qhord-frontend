...
export class ExecutionEngine {
  async execute(request: ExecutionRequestPayload, operatorId: string) {
    ...
    try {
      ...
      errorMessage =
        ...
        err?.message ||
        'Execution failed';
      if (apolloErr) responsePayload = apolloErr;
    }
  ...
  private shouldUseMockExecution(mode: ExecutionMode, apiKey: string): boolean {
  ...
  private async resolveApiKey(
    tool: ToolName,
    account: { id: string; api_key_encrypted: string }
  ): Promise<string> {
    let apiKey = '';
    try {
    ...
    await prisma.clientToolAccount.update({
      where: { id: account.id },
      data: { api_key_encrypted: encrypt(fromEnv) },
    });
    return fromEnv;
  }

  /** Map pipeline/compiler payloads to Apollo mixed_people/search shape */
  private normalizeApolloPayload(action: string, payload: Record<string, unknown>): Record<string, unknown> {
    if (action !== 'search_leads') return payload;

    if (payload.q != null) return payload;
  ...
  private buildMockExecutionResult(tool: ToolName, action: string, payload: any, mode: ExecutionMode) {
    const base = {
      mode: 'mock',
    ...
    if (tool === 'bettercontacts' && action === 'enrich_contacts') {
      const contacts = payload?.contacts || [];
      const enriched = (Array.isArray(contacts) ? contacts : []).map((c: any, i: number) => ({
        ...c,
  ...
  private async dispatchToTool(tool: ToolName, apiKey: string, action: string, payload: any): Promise<any> {
    switch (tool) {
      case 'apollo': {
        ...
        const apolloPayload =
          payload && typeof payload === 'object'
            ? this.normalizeApolloPayload(action, payload as Record<string, unknown>)
            : payload;
        if (action === 'search_leads') return service.searchLeads(apolloPayload);
      ...
      case 'hunter': {
        ...
        if (action === 'discover') return service.discover(payload);
        if (action === 'domain_search') return service.domainSearch(payload);
        if (action === 'verify_emails') return service.verifyEmails(payload);
        if (action === 'verify_email') return service.verifyEmail(payload);
        break;
      }
      case 'brevo': {
        const service = new BrevoService(apiKey);
        if (action === 'prepare_campaign') return service.prepareCampaign(payload);
        if (action === 'create_list') return service.createList(payload);
        if (action === 'create_contact') return service.createContact(payload);
        if (action === 'sync_contacts') return service.syncContacts(payload);
        if (action === 'create_campaign') return service.createCampaign(payload);
        if (action === 'send_campaign_now') return service.sendCampaignNow(payload);
        if (action === 'send_transactional') return service.sendTransactional(payload);
        break;
      }
      case 'bettercontacts': {
        const service = new BetterContactsService(apiKey);
        if (action === 'enrich_contacts') return service.enrichContacts(payload);
        if (action === 'get_enrichment_results') return service.getEnrichmentResults(payload);
        break;
      }
      case 'instantly': {
        const instService = new InstantlyService(apiKey);
        if (action === 'list_campaigns') return instService.listCampaigns(payload);
        if (action === 'create_campaign') return instService.createCampaign(payload);
        if (action === 'add_leads') return instService.addLeads(payload);
        if (action === 'get_campaign') return instService.getCampaign(payload);
        if (action === 'get_campaign_stats') return instService.getCampaignStats(payload);
        if (action === 'list_workspaces') return instService.listWorkspaces(payload);
        break;
      }
      case 'hubspot': {
        const hsService = new HubSpotService(apiKey);
        if (action === 'create_contact') return hsService.createContact(payload);
        if (action === 'search_contacts') return hsService.searchContacts(payload);
        if (action === 'get_contact') return hsService.getContact(payload);
        if (action === 'update_contact') return hsService.updateContact(payload);
        if (action === 'create_deal') return hsService.createDeal(payload);
        if (action === 'search_deals') return hsService.searchDeals(payload);
        if (action === 'get_pipelines') return hsService.getPipelines(payload);
        if (action === 'create_company') return hsService.createCompany(payload);
        if (action === 'search_companies') return hsService.searchCompanies(payload);
        break;
      }
      case 'salesforce': {
        const sfService = new SalesforceService(apiKey);
        if (action === 'query') return sfService.query(payload);
        if (action === 'search') return sfService.search(payload);
        if (action === 'create_lead') return sfService.createLead(payload);
        if (action === 'update_lead') return sfService.updateLead(payload);
        if (action === 'get_lead') return sfService.getLead(payload);
        if (action === 'create_contact') return sfService.createContact(payload);
        if (action === 'update_contact') return sfService.updateContact(payload);
        if (action === 'get_contact') return sfService.getContact(payload);
        if (action === 'create_opportunity') return sfService.createOpportunity(payload);
      ...
      case 'calendly': {
        ...
        if (action === 'get_current_user') return service.getCurrentUser();
        if (action === 'list_event_types') return service.listEventTypes(payload);
        if (action === 'create_scheduling_link') {
          ...
          if (!linkPayload.owner || !linkPayload.event_type) {
            ...
            if (!linkPayload.owner) linkPayload.owner = userUri;
        ...
        if (action === 'list_scheduled_events') return service.listScheduledEvents(payload);
        if (action === 'create_webhook_subscription') return service.createWebhookSubscription(payload);
        break;
      }
      default:
        break;
